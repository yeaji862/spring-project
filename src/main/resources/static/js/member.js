    ...
    if($('.'+data+'-phone').val() == ''){
     ...
     $('.error-phone-'+data).css('display' , '');
       ...
       const randomNum = Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000;
                       ...
                       data: { "user_phone" : $("input[name = user_phone]").val() , "randomNumber" : randomNum },
                       method: "POST",
                       dataType: "text",
                       success:function(){
                       alert('인증번호를 보냈습니다');
...
//        const CryptoJS = Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000;
//        // 0부터 100 사이의 랜덤 숫자 생성
//        const randomNumber = Math.floor(Math.random() * 100);
//        // 랜덤 숫자를 문자열로 변환하여 SHA256 해싱으로 암호화
//        const encryptedNumber = CryptoJS.SHA256(randomNumber.toString()).toString();
        $("input[name = user_pass]").val('0');
        $('.' + data).submit();
       }else{
         $('.' + data).submit();
       }
